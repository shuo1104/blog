...
function describeDay({ count, tokens, date }: Contribution) {
  const noun = count === 1 ? (tokens !== undefined ? "agent call" : "contribution") : (tokens !== undefined ? "agent calls" : "contributions");
  return `${count} ${noun}${formatTokens(tokens)} on ${DATE_FORMAT.format(new Date(`${date}T00:00:00`))}`;
}
...
async function fetchStaticContributions(login: string) {
  if (login.toLowerCase() === "agentsview") {
    try {
      const res = await fetch("/agentsview-usage.json");
      if (!res.ok) return null;
      const json = await res.json();
      if (!json || !json.contributions?.length) return null;
      return {
        contributions: json.contributions as Contribution[],
        repos: (json.repos || []) as RepoContribution[],
      };
    } catch (_) {
      return null;
    }
  ...
  try {
    ...
    if (!res.ok) return null;
    const json = await res.json();
    const userData = json?.users?.[login.toLowerCase()];
    if (!userData || !userData.contributions || !userData.contributions.length) {
      return null;
    }
    return {
      contributions: userData.contributions as Contribution[],
      repos: (userData.repos || []) as RepoContribution[],
    };
  } catch (_) {
    return null;
  }
...
function useGitHubUser(login?: string) {
  ...
  React.useEffect(() => {
    ...
    async function loadData() {
      // 1. Instantaneous Static Snapshot (0ms, offline-proof & rate-limit proof)
      const staticData = await fetchStaticContributions(login!);
      if (active && staticData) {
        setData(staticData);
        return;
      }

      if (login!.toLowerCase() === "agentsview") return;

      // 2. Fallback to live public APIs
      try {
...
function generateFallbackDays(weeks: number, username = "shuo"): Contribution[] {
  ...
  return Array.from({ length: totalDays }, (_, i) => {
    ...
    if (!isWeekend) {
      if (r > 0.32) {
        count = Math.floor(pseudoRandom() * 7) + 1;
        level = (count >= 5 ? 4 : count >= 3 ? 3 : count >= 2 ? 2 : 1) as ContributionLevel;
      }
    } else {
      if (r > 0.6) {
        count = Math.floor(pseudoRandom() * 4) + 1;
        level = (count >= 3 ? 3 : count >= 2 ? 2 : 1) as ContributionLevel;
      }
    }
...
const GitHubActivity = ({
  ...
  const placeholder = React.useMemo(
    () => (username ? generateFallbackDays(weeksFor(months), username) : []),
    [username, months],
  );
  ...
  const fallbackRepos: RepoContribution[] = [
    { name: `${username || "shuo"}/shuo-blog`, count: 148, href: `https://github.com/${username || "shuo"}` },
    { name: `${username || "shuo"}/core-kernel`, count: 82, href: `https://github.com/${username || "shuo"}` },
    { name: `${username || "shuo"}/serene-ui`, count: 46, href: `https://github.com/${username || "shuo"}` },
  ];
  const repos = reposProp.length ? reposProp : ((fetched?.repos && fetched.repos.length > 0) ? fetched.repos : fallbackRepos);
  ...
  const isAgent = username?.toLowerCase() === "agentsview";
  const formattedTotal = new Intl.NumberFormat("en-US").format(total);
  const defaultHeading = isAgent
    ? `${formattedTotal} agent calls (3.32B tokens)${displayYear ? ` in ${displayYear}` : ""}`
    : `${formattedTotal} contributions${displayYear ? ` in ${displayYear}` : ""}`;
  const heading = headingProp ?? defaultHeading;
  ...
  return (
    <LayoutGroup id={uid}>
      <div
        ...
        className={cn(
          "relative w-full max-w-full overflow-hidden rounded-3xl border border-border/80 bg-neutral-50/50 p-5 sm:p-6 font-sans shadow-xs select-none dark:border-neutral-800/80 dark:bg-neutral-900/40 backdrop-blur-xs flex flex-col justify-between",
          className,
        )}
        style={style}
        {...props}
      >
        <div>
          <p className="mb-4 text-base font-medium text-foreground px-1">
            {heading}
          </p>
        ...
        {repos.length > 0 && (
          <motion.div
            ...
            className="mt-5 w-full overflow-hidden rounded-2xl border border-neutral-200/90 bg-white/95 shadow-2xs backdrop-blur-xl dark:border-neutral-700/60 dark:bg-neutral-850/95"
          ...
          >
            <motion.div
              ...
              className="flex items-center justify-between gap-3 py-2.5 px-3.5"
            >
              ...
              <div className="flex items-center gap-2.5">
                ...
                <button
                  ...
                  className="grid size-6.5 shrink-0 place-items-center rounded-full bg-card cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"
                >
            ...
            <AnimatePresence initial={false} mode="popLayout">
              {open && (
                <motion.ul
                  ...
                  className="px-1 pb-2 flex flex-col gap-0.5"
                >
