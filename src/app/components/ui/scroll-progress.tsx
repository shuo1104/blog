...
export type ScrollProgressProps = React.ComponentProps<"div"> & {
  ...
  offset?: number;
  placement?: "left-sidebar" | "bottom" | "top-left";
};
...
const ScrollProgress = ({
  ...
  const radius = open ? 22 : (collapsedSize?.height ?? 32) / 2;
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none invisible absolute" aria-hidden>
        <div
          ref={collapsedRef}
          className="inline-flex items-center gap-2.5 py-1.5 pl-2.5 pr-4 border border-transparent"
        >
        ...
        <div ref={openRef} className="w-max max-w-[280px] p-1.5 border border-transparent">
          {sections.map((s) => (
            <div
              key={s.id}
      ...
      {size && (
        <motion.div
          ...
          className={cn(
            "overflow-hidden border border-neutral-200/80 bg-white/95 shadow-md backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/95",
            isBottom
          ...
          <AnimatePresence initial={false} mode="popLayout">
            {open ? (
                  ...
                  return (
                    <li key={s.id}>
                      <button
                        ...
                        className={cn(
                          "relative flex w-full items-center gap-3 rounded-[12px] px-3 py-2 text-left text-sm font-medium leading-none transition-colors cursor-pointer",
                          squircle,
                          isActive
                      ...
                      >
                        {isActive && (
                          <motion.span
                            ...
                            className={cn(
                              "absolute inset-0 rounded-[12px] bg-neutral-200/60 dark:bg-neutral-800/80",
                              squircle
                            )}
                            transition={
              ...
              <motion.button
                ...
                className="absolute inset-0 flex items-center gap-2.5 py-1.5 pl-2.5 pr-4 cursor-pointer select-none"
                initial={{
                  opacity: 0,
                  filter: reduceMotion ? undefined : "blur(4px)",
                }}
